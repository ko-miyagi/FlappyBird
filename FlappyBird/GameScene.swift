...
//  FlappyBird
//
//  Created by 宮城 光太朗 on 2023/05/16.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let itmScoreCategory: UInt32   = 1 << 4     // 0...10000
    ...
    // アイテムスコア用
    var itemScore = 0
    var itmScoreLabelNode:SKLabelNode!
    ...
    override func didMove(to view: SKView) {
        ...
        //音声ファイル読み込み
//        if let soundURL = Bundle.main.url(forResource: "sound_egg", withExtension: "mp3") {
//            let audioNode = SKAudioNode(url: soundURL)
//            self.addChild(audioNode)
//        }
        soundAction = SKAction.playSoundFileNamed("sound_egg.mp3", waitForCompletion: false)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        } else if (contact.bodyA.categoryBitMask & itmScoreCategory) == itmScoreCategory || (contact.bodyB.categoryBitMask & itmScoreCategory) == itmScoreCategory {
            // スコアカウント用の透明な壁と衝突した
            print("itemScoreUp")
            itemScore += 1
            itmScoreLabelNode.text = "Item:\(itemScore)"
            
            //効果音を鳴らす
            self.run(soundAction)
            
            let removeItem = SKAction.removeFromParent()
            if (contact.bodyA.categoryBitMask & itmScoreCategory) == itmScoreCategory {
                // 自身を取り除くアクションを作成
                contact.bodyA.node?.run(removeItem)
            }
            else {
    ...
    func setupItem() {
        //アイテム画像を読み込む
        let itemTexture = SKTexture(imageNamed: "item_egg")
        ...
        let createItemAnimation = SKAction.run({
            ...
            itemSpriteNode.physicsBody?.categoryBitMask = self.itmScoreCategory
        ...
        //let itemWaitCreate = SKAction.wait(forDuration: 1)
    ...
    func setupBird() {
        ...
        bird.physicsBody?.collisionBitMask = groundCategory | wallCategory
        bird.physicsBody?.contactTestBitMask = groundCategory | wallCategory | scoreCategory | itmScoreCategory
    ...
    func setupScoreLabel() {
        ...
        // アイテムスコア表示を作成
        itemScore = 0
        itmScoreLabelNode = SKLabelNode()
        itmScoreLabelNode.fontColor = UIColor.black
        itmScoreLabelNode.position = CGPoint(x: 10, y: self.frame.size.height - 120)
        itmScoreLabelNode.zPosition = 100 // 一番手前に表示する
        itmScoreLabelNode.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.left
        itmScoreLabelNode.text = "Item:\(itemScore)"
        self.addChild(itmScoreLabelNode)
    }
}
